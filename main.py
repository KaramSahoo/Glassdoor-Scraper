...
def scrape(field, review, author):
    ...
    def scrape_date(review):
        res = review.find_element(by=By.CLASS_NAME, value='authorInfo').text.split('-')[0]
        return res

    def scrape_emp_title(review):
        if 'Anonymous Employee' not in review.text:
            try:
                res = review.find_element(by=By.CLASS_NAME, value='authorInfo').text.split('-')[1]
            except Exception:
    ...
    #         try:
    #             res = author.find_element(by=By.CLASS_NAME, value=
    #                 'authorLocation').text
    #         except Exception:
    ...
    def scrape_rev_title(review):
        return review.find_element(by=By.CLASS_NAME, value='reviewLink').text

    # def scrape_helpful(review):
    #     try:
    #         helpful = review.find_element(by=By.CLASS_NAME, value='helpfulCount')
    ...
    def scrape_pros(review):
        try:
            pros = review.find_element(by=By.XPATH,value=".//span[@data-test='pros']")
            res = pros.text
        except Exception:
            res = ""
        return res
    ...
    def scrape_cons(review):
        try:
            cons = review.find_element(by=By.XPATH,value=".//span[@data-test='cons']")
            res = cons.text
        except Exception:
            res = ""
        return res
    ...
    def scrape_overall_rating(review):
        try:
            ratings = review.find_element(by=By.CLASS_NAME, value='ratingNumber')
            res = float(ratings.text)
        except Exception:
            res = np.nan
        return res
    ...
    #     try:
    #         ratings = review.find_element(by=By.CLASS_NAME, value='gdStars')
    #         subratings = ratings.find_element(by=By.CLASS_NAME, value=
    #             'subRatings').find_element(by=By.TAG_NAME, value='ul')
    #         this_one = subratings.find_elements(by=By.TAG_NAME, value='li')[i]
    #         res = this_one.find_element(by=By.CLASS_NAME, value=
    ...
    def scrape_recommends(review):
        try:
            recommends = review.find_element(by=By.CLASS_NAME, value='recommends')
            subRecom = recommends.find_elements(by=By.CLASS_NAME, value='d-flex.align-items-center.mr-std')
            # print([i.get_attribute('class') for i in subRecom[1].find_elements_by_xpath("./*")])
            res = subRecom[0].find_elements_by_xpath("./*")[0].get_attribute('class').split()[1]
            return colorDict[res]
        except:
            return np.nan
    ...
    def scrape_outlook(review):
        try:
            ...
            subRecom = recommends.find_elements(by=By.CLASS_NAME, value='d-flex.align-items-center.mr-std')
            # print([i.get_attribute('class') for i in subRecom[1].find_elements_by_xpath("./*")])
            res = subRecom[2].find_elements_by_xpath("./*")[0].get_attribute('class').split()[1]
            return colorDict[res]
        except:
            return np.nan
    ...
    def scrape_approve_ceo(review):
        try:
            ...
            subRecom = recommends.find_elements(by=By.CLASS_NAME, value='d-flex.align-items-center.mr-std')
            # print([i.get_attribute('class') for i in subRecom[1].find_elements_by_xpath("./*")])
            res = subRecom[1].find_elements_by_xpath("./*")[0].get_attribute('class').split()[1]
            return colorDict[res]
        except:
            return np.nan
...
def navigate_to_reviews():
    ...
    reviews_cell = browser.find_element(by=By.XPATH, value='//a[@data-label="Reviews"]')
    reviews_path = reviews_cell.get_attribute('href')
    logger.info(reviews_path)
...
def sign_in():
    ...
    url = 'https://www.glassdoor.co.in/profile/login_input.htm?userOriginHook=HEADER_SIGNIN_LINK'
    browser.get(url)
    
    email_field = browser.find_element(by=By.NAME, value='username')
    password_field = browser.find_element(by=By.NAME, value='password')
    submit_btn = browser.find_element(by=By.NAME, value='submit')
